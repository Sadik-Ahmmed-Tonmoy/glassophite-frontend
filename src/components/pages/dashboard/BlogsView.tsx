...
const blogSchema = z.object({
  ...
  content: z.string().min(30, "Article content must be at least 30 characters."),
...
export default function BlogsView() {
  ...
  const [imageUrl, setImageUrl] = useState("https://images.unsplash.com/photo-1511499767150-a48a237f0083?auto=format&fit=crop&w=900&q=80");
  ...
  const resetForm = () => {
    ...
    setImageUrl("https://images.unsplash.com/photo-1511499767150-a48a237f0083?auto=format&fit=crop&w=900&q=80");
    setStatus("Published");
    setFeatured(false);
    setFormErrors({});
  };
  ...
  const handleSavePost = (event: React.FormEvent) => {
    ...
    if (slugAlreadyExists) {
      setFormErrors({ slug: "This slug is already used by another blog post." });
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="glass-panel rounded-2xl border border-border p-4">
          <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-extrabold">Total Posts</p>
          <p className="text-2xl font-extrabold mt-1">{posts.length}</p>
        </div>
        <div className="glass-panel rounded-2xl border border-border p-4">
          <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-extrabold">Published</p>
          <p className="text-2xl font-extrabold mt-1">{posts.filter((post) => post.status === "Published").length}</p>
        </div>
        <div className="glass-panel rounded-2xl border border-border p-4">
          <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-extrabold">Featured</p>
          <p className="text-2xl font-extrabold mt-1">{posts.filter((post) => post.featured).length}</p>
        </div>
      </div>
                ...
                <tr key={post.id} className="hover:bg-muted/20 transition-colors">
                  <td className="p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-extrabold text-foreground">{post.title}</p>
                        <p className="text-[10px] text-muted-foreground font-mono">/{post.slug}</p>
                        <p className="text-[10px] text-muted-foreground mt-1">{post.date} / {post.readTime}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-4 font-bold text-primary">{post.category}</td>
                  <td className="p-4 text-muted-foreground font-semibold">{post.author}</td>
                  <td className="p-4">
                    <span className={`px-2.5 py-0.5 text-[10px] font-bold rounded-full ${post.status === "Published" ? "bg-green-500/10 text-green-600 dark:text-green-400" : "bg-muted text-muted-foreground border border-border"}`}>
                      {post.status}
                    </span>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <div className="flex justify-center gap-1.5">
                      ...
                      <button
                        onClick={() => setDeleteConfirm({ isOpen: true, id: post.id, title: post.title })}
              ...
              <form onSubmit={handleSavePost} className="space-y-4 text-xs">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <Field label="Author" error={formErrors.author}>
                    <input value={author} onChange={(event) => setAuthor(event.target.value)} className="dashboard-input" />
                  </Field>
                  <Field label="Publish Date" error={formErrors.date}>
                    <input type="date" value={date} onChange={(event) => setDate(event.target.value)} className="dashboard-input" />
                  </Field>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-[1fr_160px_160px] gap-4">
                  <Field label="Image URL" error={formErrors.imageUrl}>
                    <div className="relative">
                      ...
                      <input value={imageUrl} onChange={(event) => setImageUrl(event.target.value)} className="dashboard-input pl-9" />
                    </div>
                  </Field>
                  <Field label="Read Time" error={formErrors.readTime}>
                    <input value={readTime} onChange={(event) => setReadTime(event.target.value)} className="dashboard-input" />
                  </Field>
                  <Field label="Status" error={formErrors.status}>
                    <select value={status} onChange={(event) => setStatus(event.target.value as "Published" | "Draft")} className="dashboard-input">
